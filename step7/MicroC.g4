...
base_type returns [Scope.Type t]: 'int' {$t = new Scope.Type(Scope.InnerType.INT);}| 'float' {$t = new Scope.Type(Scope.InnerType.FLOAT);} | 'void' {$t = new Scope.Type(Scope.InnerType.VOID);};
...
if_stmt returns [IfStatementNode node]: 'if' '(' cond ')' '{' statements '}' if_else {$node = new IfStatementNode($cond.node, $statements.node, $if_else.node);};

if_else returns [StatementListNode node]: 'else' '{' statements '}' { $node = new StatementListNode($statements.node);}
          | /* empty */ {$node = new StatementListNode();};
...
ptr_expr returns [PtrDerefNode node] : '*' primary {$node = new PtrDerefNode($primary.node);};

cast_expr returns [CastNode node] : '(' type ')' expr {$node = new CastNode($type.t, $expr.node);}; //TODO check

addr_of_expr returns [AddrOfNode node] : '&' lval {$node = new AddrOfNode($lval.node);}
          | '&' array_expr {$node = new AddrOfNode($array_expr.node);};

array_expr returns [PtrDerefNode node] : lval '[' expr ']' {$node = new PtrDerefNode(new BinaryOpNode($lval.node, new BinaryOpNode(new IntLitNode("4"), $expr.node, "*"), "+"));}
        | ae=array_expr '[' expr ']'{$node = new PtrDerefNode(new BinaryOpNode($ae.node, new BinaryOpNode(new IntLitNode("4"), $expr.node, "*"), "+"));};
...
term returns [ExpressionNode node] : primary {$node = $primary.node;}
     ...
     | cast_expr {$node = $cast_expr.node;}; //TODO check
